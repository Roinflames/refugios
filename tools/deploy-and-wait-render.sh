...
BASE_URL="${1:-${APP_URL:-}}"
HOOK_URL="${2:-${RENDER_DEPLOY_HOOK_URL:-}}"
INTERVAL="${RENDER_WAIT_INTERVAL:-10}"
TIMEOUT="${RENDER_WAIT_TIMEOUT:-900}"

if [ -z "${BASE_URL}" ]; then
  echo "Uso: APP_URL=https://tu-app.onrender.com RENDER_DEPLOY_HOOK_URL=... $0"
  echo "  o: $0 https://tu-app.onrender.com https://api.render.com/deploy/srv-...?key=..."
  exit 2
fi

if [ -z "${HOOK_URL}" ]; then
  echo "Falta Deploy Hook URL."
  echo "Define RENDER_DEPLOY_HOOK_URL o pasalo como segundo argumento."
  exit 2
fi
